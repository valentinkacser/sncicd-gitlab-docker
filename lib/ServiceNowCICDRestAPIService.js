...
function ServiceNowCICDRestAPIService(instance, auth, transport = null) {
    ...
    this.testSuiteCloudRun = testSuiteCloudRun;
    ...
    function getTSResults(resultID) {
        ...
        /**
     * Get Cloud Runner TestSuite run results by ID
     * @param resultID string
     * @returns {Promise<string>}
     */
    function getCLTSResults(resultID) {
        const URL = `https://${config.instance}/api/now/table/sys_execution_tracker/` + resultID;
        return request(URL).then(async (resp) => {
                console.log(resp);
                let resultObj = JSON.parse(resp.result);
                resultToReturn = resultObj.test_suite_result_id ? resultObj.test_suite_result_id : resultObj.result_id;
                console.log("Result id is:" + resultToReturn);
                return resultToReturn;
            })
            .catch(err => Promise.reject(err.errorMessage)).
            then(resultID => getTSResults(resultID));
    }
    ...
    /**
     * Run Test Suite in Cloud Runner
     * available options are:
     * test_suite_sys_id
     * required options are:
     * test_suite_sys_id
    ...
    function testSuiteCloudRun(options) {
        if (!options || !(options.test_suite_sys_id)) {
            return Promise.reject('Please specify test_suite_sys_id');
        }
        return request(
            `https://${config.instance}/api/now/sn_atf_tg/test_runner`,
            '',
            'POST',
            `{ "testId":"${options.test_suite_sys_id}"}`)
            .then(resp => getCLProgress(resp))
            .catch(err => (err.response.results && err.response.results.id) ? 
                err.response : Promise.reject(err.errorMessage))
            .then(resp => getPropertyByPath(resp, 'root_tracker_id.value')).
            then(resultID => resultID ? getCLTSResults(resultID) : Promise.reject('No result ID'));
    }
...
///////////////////////////////////////////////////////////////////////////////
     ...
     */
     function getProgress(response, returnProgress = false) {
        ...
        if (progressId && (status === 0 || status === 1)) {
            ...
            //process.stdout.write(`ProgressId: ${progressId}`);
            // This is an etra comment line
    ...
    //function initiateCLProgress(response){
    //    return getPropertyByPath(response, 'snboqId');
        /*console.log(`Initiated progress id ${progressId}`);
        if (progressId) {
            const progressUrl = `https://${config.instance}/api/now/table/sn_atf_tg_sn_boq/` + progressId;
        return request(progressUrl).then(data => {
            return (getPropertyByPath(data, 'snboqId')) || false;
        }).catch(() => false);
    }*/
     ...
     * @returns {Promise<string>|<object>}
     */
    function getCLProgress(response, returnProgress = false) {
        //console.log(response);
        let snoqid =getPropertyByPath(response, 'snboqId');
        let progressId = snoqid ? snoqid :  getPropertyByPath(response, 'sys_id');
        let status = snoqid ? "none" : getPropertyByPath(response, 'state');
        //const progressId = getPropertyByPath(response, 'sys_id');
        //console.log(snoqid+' '+status+' '+progressId);
        if (progressId && (status === "running" || status === "browsers_requested" || status === "none" || status === "processing" || status === "pending")) {
            const progressUrl = `https://${config.instance}/api/now/table/sn_atf_tg_sn_boq/` + progressId;
            //console.log(progressUrl+' '+status);
            return request(progressUrl)
                .then(progressBody => {
                    status = progressBody.state;
                    //response.results = getPropertyByPath(progressBody, 'root_tracker_id');
                    if (status === "completed") {
                            return Promise.resolve(returnProgress ? progressBody : response);   
                    } else {
                        if (status === "running") {
                            const percentage = getPropertyByPath(progressBody, 'progress');
                            if (percentage) {
                                process.stdout.write(`${percentage}% complete`);
                            }
                        }
                        if (status === "browsers_requested") {
                            process.stdout.write("Requesting Browser");
                        }
                        if (status === "processing") {
                            process.stdout.write("Processing");
                        }
                        if (status === "pending") {
                            process.stdout.write("Test run status: Pending");
                        }
                        process.stdout.write('.\n');
                        return wait(config.delayInProgressPolling).then(() => getCLProgress(progressBody, returnProgress));
                    }
                });
        } else {
